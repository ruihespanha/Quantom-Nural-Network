...
base = r"/Users/siddharthvedam/Downloads/Track 7---SRA/Quantom-Nural-Network/ALL DATA"
glioma_images,   glioma_labels   = load_images_from_folder(os.path.join(base, "Glioma"),    0)
meningioma_images, meningioma_labels = load_images_from_folder(os.path.join(base, "Meningioma"),1)
pituitary_images,  pituitary_labels  = load_images_from_folder(os.path.join(base, "Pituitary"), 2)

# combine into single arrays
x_raw = np.stack(glioma_images + meningioma_images + pituitary_images)  # shape (ΣN,64,64)
...
for i in range(epochs):
    ...
    print(f"Iter: {i+1:5d} | Cost: {cost(point, X_train, Y_train):0.7f} | Acc train: {acc_train:0.3f}")
